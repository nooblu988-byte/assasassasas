...
module.exports = {
    ...
    run: async (client, message, args, prefix) => {
        ...
        if (!input) {
            return message.channel.send(errorContainer(`${client.emoji.error} | **Invalid Usage** — \`${prefix}unban @user/username/ID [reason]\``));
        }

        const banList = await message.guild.bans.fetch().catch(() => null);

        if (!banList) {
            return message.channel.send(errorContainer("${client.emoji.error} | Failed to fetch the ban list. Please check my permissions."));
        }
        ...
        if (!banEntry) {
            return message.channel.send(errorContainer(`${client.emoji.error} | No banned user was found matching **${input}**.`));
        }
        ...
        const successContainer = new ContainerBuilder()
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent(`${client.emoji.tick2} | **[${user.username}](http://discord.com/users/${user.id})** [\`${user.id}\`] has been unbanned from the server.\n**Reason:** ${reason}`)
            );
